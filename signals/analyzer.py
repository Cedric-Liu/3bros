...
from utils.indicators import calculate_macd, calculate_ma, check_macd_cross
...
class StrategyAnalyzer:
    ...
    def analyze(self, df: pd.DataFrame, code: str, name: str) -> Optional[StrategyAnalysis]:
        ...
        upper_shadow_warning = upper_shadow_ratio > 0.5
    ...
    def _analyze_volume(self, df: pd.DataFrame) -> tuple:
        ...
        if ratio > 1.5:
            return "放量", round(ratio, 2)
        elif ratio < 0.7:
            return "缩量", round(ratio, 2)
        else:
            return "平量", round(ratio, 2)
    ...
    def _is_new_high(self, df: pd.DataFrame, period: int) -> bool:
        ...
        return df["high"].iloc[-1] >= recent_high * 0.99
    ...
    def _is_new_low(self, df: pd.DataFrame, period: int) -> bool:
        ...
        return df["low"].iloc[-1] <= recent_low * 1.01
    ...
    def _calculate_support_resistance(self, df: pd.DataFrame) -> tuple:
        """
        计算压力线和支撑线
        基于PPT: 大阴线/大阳线的开盘价、1/3、1/2位置
        返回详细信息包括：参考日期、计算方式、具体价位、与现价对比
        会往前找60天的数据
        """
        supports = []
        resistances = []
        ...
        # 往前找60天的数据，找最近的大阴线和大阳线
        search_range = min(60, len(df))
        found_resistance = False
        found_support = False

        for i in range(-1, -search_range - 1, -1):
            if found_resistance and found_support:
                break
            ...
            # 大阴线 (跌幅超过2.5%) -> 产生压力线
            if pct_change < -2.5 and not found_resistance:
                drop = row["open"] - row["close"]
                open_price = round(row["open"], 2)
                half_price = round(row["close"] + drop * 0.5, 2)
                third_price = round(row["close"] + drop / 3, 2)
                ...
                def calc_distance(price):
                    ...
                    if diff > 0:
                        return f"高于现价{abs(pct):.1f}%"
                    else:
                        return f"已突破，低于现价{abs(pct):.1f}%"
                ...
                resistances.append({
                    ...
                    "days_ago": days_ago,
                    "ref_open": open_price,
                    "ref_close": round(row["close"], 2),
                    "calculation": f"开盘价{open_price}",
                    "vs_current": calc_distance(open_price)
                })
                resistances.append({
                    "price": half_price,
                    "name": "跌幅1/2处(中压力)",
                    ...
                    "ref_open": open_price,
                    "ref_close": round(row["close"], 2),
                    "calculation": f"收盘{row['close']:.2f}+跌幅一半",
                    "vs_current": calc_distance(half_price)
                })
                resistances.append({
                    "price": third_price,
                    "name": "跌幅1/3处(弱压力)",
                    ...
                    "ref_open": open_price,
                    "ref_close": round(row["close"], 2),
                    "calculation": f"收盘{row['close']:.2f}+跌幅三分之一",
                    "vs_current": calc_distance(third_price)
                })
                found_resistance = True

            # 大阳线 (涨幅超过2.5%) -> 产生支撑线
            elif pct_change > 2.5 and not found_support:
                gain = row["close"] - row["open"]
                open_price = round(row["open"], 2)
                half_price = round(row["open"] + gain * 0.5, 2)
                third_price = round(row["open"] + gain / 3, 2)

                # 计算与现价的距离
                def calc_distance_support(price):
                    diff = current_price - price
                    pct = (diff / current_price) * 100
                    if diff > 0:
                        return f"现价高于此位{abs(pct):.1f}%"
                    else:
                        return f"已跌破，现价低于此位{abs(pct):.1f}%"
                ...
                supports.append({
                    ...
                    "ref_close": round(row["close"], 2),
                    "calculation": f"开盘价{open_price}",
                    "vs_current": calc_distance_support(open_price)
                })
                supports.append({
                    "price": half_price,
                    "name": "涨幅1/2处(中支撑)",
                    ...
                    "ref_open": open_price,
                    "ref_close": round(row["close"], 2),
                    "calculation": f"开盘{open_price}+涨幅一半",
                    "vs_current": calc_distance_support(half_price)
                })
                supports.append({
                    "price": third_price,
                    "name": "涨幅1/3处(弱支撑)",
                    ...
                    "ref_open": open_price,
                    "ref_close": round(row["close"], 2),
                    "calculation": f"开盘{open_price}+涨幅三分之一",
                    "vs_current": calc_distance_support(third_price)
                })
                found_support = True
    ...
    def _analyze_ma(self, df: pd.DataFrame) -> Dict:
        ...
        for period in self.ma_periods:
            if len(df) >= period:
                ma_value = calculate_ma(df["close"], period).iloc[-1]
